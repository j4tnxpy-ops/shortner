...
export const signUp = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<PublicProfile> => {
    ...
    const { hashPassword, createSessionCookie, randomReferralCode, randomApiKey } = await import(
      "./auth.server"
    );
...
export const signIn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<PublicProfile> => {
    ...
    if (!apiKey) {
      const { randomApiKey } = await import("./auth.server");
      apiKey = randomApiKey();
      await users.updateOne({ _id: user._id }, { $set: { api_key: apiKey } });
    }
    return {
      id: String(user._id),
      email: user.email,
...
export const rotateApiKey = createServerFn({ method: "POST" }).handler(async () => {
  const { requireUserId } = await import("./auth.server");
  const { collections } = await import("./db.server");
  const { ObjectId } = await import("mongodb");
  const { randomApiKey } = await import("./auth.server");
  ...
  await users.updateOne({ _id: new ObjectId(userId) }, { $set: { api_key: apiKey } });
...
export const getAdSettings = createServerFn({ method: "GET" }).handler(async () => {
  const { collections } = await import("./db.server");
  const { settings } = await collections();
  const docs = await settings.find({ _id: { $in: ["ad_banner_html", "ad_interstitial_html", "ad_popup_html"] } }).toArray();
  return Object.fromEntries(docs.map((d) => [d._id, d.value])) as Record<string, string>;
});
