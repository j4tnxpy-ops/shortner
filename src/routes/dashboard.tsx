      ...
      <section className="mx-auto max-w-6xl px-4 py-10">
        <Tabs defaultValue="links">
          ...
          <TabsContent value="api" className="mt-6">
            <Card>
              <CardHeader><CardTitle>Developer API</CardTitle><CardDescription>Use your private API key to create short links programmatically.</CardDescription></CardHeader>
              <CardContent className="space-y-4">
                <div className="rounded-lg border bg-muted/40 p-4 font-mono text-sm break-all">{profile?.api_key || "No API key"}</div>
                <Button variant="outline" onClick={() => void (async () => { try { const r = await rotateApiKey(); await refreshProfile(); toast.success("API key regenerated"); void navigator.clipboard?.writeText(r.api_key); } catch (e) { toast.error(e instanceof Error ? e.message : "Could not regenerate key"); } })()}>Regenerate API key</Button>
                <p className="text-xs text-muted-foreground">POST /api/public/shorten with JSON: {`{"token":"YOUR_API_KEY","url":"https://example.com"}`}</p>
              </CardContent>
            </Card>
          </TabsContent>
