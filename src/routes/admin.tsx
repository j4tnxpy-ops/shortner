...
function AdminPage() {
  ...
  useEffect(() => {
    ...
    setBannerAd(settings.data["ad_banner_html"] ?? "");
    setInterstitialAd(settings.data["ad_interstitial_html"] ?? "");
    setPopupAd(settings.data["ad_popup_html"] ?? "");
  }, [settings.data]);
          ...
          <TabsContent value="ads" className="mt-6 space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Paste trusted ad-network HTML/JavaScript snippets. These snippets are rendered on the interstitial/redirect page.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-1.5"><Label htmlFor="banner-ad">Banner ad HTML</Label><Textarea id="banner-ad" rows={5} value={bannerAd} onChange={(e) => setBannerAd(e.target.value)} placeholder="Paste your banner ad code here" /></div>
                <div className="space-y-1.5"><Label htmlFor="interstitial-ad">Interstitial ad HTML</Label><Textarea id="interstitial-ad" rows={7} value={interstitialAd} onChange={(e) => setInterstitialAd(e.target.value)} placeholder="Paste your interstitial ad code here" /></div>
                <div className="space-y-1.5"><Label htmlFor="popup-ad">Popup ad HTML</Label><Textarea id="popup-ad" rows={5} value={popupAd} onChange={(e) => setPopupAd(e.target.value)} placeholder="Optional popup code" /></div>
                <Button className="w-full" onClick={() => void run(async () => {
                  await adminSaveSetting({ data: { key: "ad_banner_html", value: bannerAd } });
                  await adminSaveSetting({ data: { key: "ad_interstitial_html", value: interstitialAd } });
                  await adminSaveSetting({ data: { key: "ad_popup_html", value: popupAd } });
                }, "Advertisement settings saved", ["settings"])}>Save advertisement settings</Button>
                <p className="text-xs text-muted-foreground">Only paste code from an ad network you trust. Never paste unknown scripts.</p>
              </CardContent>
            </Card>
          </TabsContent>
        </Tabs>
