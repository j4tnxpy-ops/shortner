...
import { resolveAlias, recordClick, getAdSettings } from "@/lib/api.functions";
...
function AliasPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      const [link, adSettings] = await Promise.all([resolveAlias({ data: { alias } }), getAdSettings()]);
      if (!active) return;
      if (!link) {
        setState("missing");
        return;
    ...
    <div className="flex min-h-screen flex-col bg-soft-gradient">
      ...
      <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col items-center justify-center gap-6 px-4 py-12 text-center">
        ...
        <div className="flex h-32 w-full max-w-xl items-center justify-center rounded-xl border border-dashed border-border bg-card text-xs text-muted-foreground">
          {ads["ad_interstitial_html"] ? <AdSlot html={ads["ad_interstitial_html"]} /> : "Advertisement space"}
        </div>

        {ads["ad_banner_html"] && <AdSlot html={ads["ad_banner_html"]} className="w-full max-w-xl" />}
        {ads["ad_popup_html"] && <AdSlot html={ads["ad_popup_html"]} className="hidden" />}
